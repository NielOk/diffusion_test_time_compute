...
class MNISTDiffusion(nn.Module):
    def __init__(self,image_size,in_channels,num_classes,time_embedding_dim=256,timesteps=1000,base_dim=32,dim_mults= [1, 2, 4, 8]):
        ...
        self.model=Unet(timesteps,time_embedding_dim,num_classes,in_channels,in_channels,base_dim,dim_mults)
    ...
    @torch.no_grad()
    def sampling(self, n_samples, labels, clipped_reverse_diffusion=True, device=None):
        ...
        for i in tqdm(range(self.timesteps-1, -1, -1), desc="Sampling"):
            ...
            if clipped_reverse_diffusion:
                x_t = self._reverse_diffusion_with_clip(x_t, t, noise, labels)
            else:
                x_t = self._reverse_diffusion(x_t, t, noise, labels)
