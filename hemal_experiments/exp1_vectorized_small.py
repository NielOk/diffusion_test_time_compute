...
# -----------------------------------------------------------------------------
# 1) FID SUPPORT: Toy "MNIST Inception" + feature extraction + FID
# -----------------------------------------------------------------------------
...
def create_digit_dataloader(digit, subset_size=None, batch_size=128, image_size=28, num_workers=0):
    """
    Load MNIST samples for a specific digit (from TRAIN set).
    If subset_size is given, randomly select that many samples for the digit.
    """
...
def score_candidates(approach, dist_key, t, candidates):
    ...
    Returns a 1D tensor of scores (higher is better), shape [B, K].
    """
    ...
    if approach == "mse":
        ...
        target = target_t.expand(B, K, -1, -1, -1).reshape(B * K, C, H, W)
        ...
        if t not in posterior_means:
            return None
        mu_t = posterior_means[t].expand(B, K, -1, -1, -1).reshape(B * K, C, H, W)
        ...
        sq_dist = diff.pow(2).mean(dim=[3,4,5])  # shape (B, K, N)
        exponent = -sq_dist / (2.0 * var_t)

        # Numerically stable log-sum-exp across the mixture
        ...
        ll = torch.log(mixture_sum + 1e-12)   # log-likelihood
...
def top_k_search(model, dist_key, approach, n_candidates=64, device="cuda", batch_size=16):
    ...
    for t in range(model.timesteps - 1, -1, -1):
        ...
        if scores_t is not None:
            k = max(1, K // 2)
            topk_indices = torch.topk(scores_t, k=k, dim=1).indices  # (B, k)
...
def get_path_checkpoints(num_steps, delta_f, delta_b):
    """
    For 'paths' search, define partial forward/back steps by chunking timesteps.
    """
    checkpoints = []
...
def search_over_paths(
    ...
    for ckpt_t in rev_checkpoints:
        B, K, C, H, W = candidates.shape
        # Reverse diffuse until we reach ckpt_t
        ...
        if scores_ckpt is not None:
            ...
            topk_indices = torch.topk(scores_ckpt, k=k, dim=1).indices
            candidates = torch.gather(
                candidates, dim=1,
                index=topk_indices.unsqueeze(-1).unsqueeze(-1).unsqueeze(-1).expand(-1, -1, C, H, W)
            )
        ...
        candidates = candidates.repeat_interleave(n_candidates, dim=1)
        noise = torch.randn_like(candidates)
...
def generate_samples_for_digit(
    ...
    num_batches = (n_experiments + batch_size - 1) // batch_size  # ceil division
    for batch_idx in range(num_batches):
        chunk_size = min(batch_size, n_experiments - len(out_samples))
        if chunk_size <= 0:
            break

        current_count = len(out_samples)
        print(f"   [Digit {digit}] Generating {chunk_size} samples "
              f"(so far {current_count}/{n_experiments}) with approach={approach}, search={search_method}")
...
def run_scaling_study(
    ...
    Also compute FID if `fid_model` is provided.
    Returns { subset_size : (accuracy, fid) } if FID is computed,
    otherwise { subset_size : accuracy }.
    """
    results = {}
...
def main():
    ...
    # Load our toy MNIST "Inception" for FID
    ...
    with open(full_log_path, "w") as f_log:
        ...
        results = run_scaling_study(
            ...
            device=device,
            fid_model=fid_inception_model,   # <--- Pass your FID model here
        )
        ...
        with open(approach_result_path, "w") as f_approach:
            ...
            for sz, val in results.items():
                if isinstance(val, tuple):
                    ...
                    f_approach.write(f"  subset_size={sz}, accuracy={val:.4f}\n")
