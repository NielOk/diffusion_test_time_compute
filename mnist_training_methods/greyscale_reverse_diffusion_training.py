...
def load_non_noisy_data(training_data_path, batch_size=4):
    ...
    X_train_batches, y_train_batches = generator.create_batches(X_train, y_train, batch_size=batch_size)
    X_test_batches, y_test_batches = generator.create_batches(X_test, y_test, batch_size=batch_size)
...
def visualize_model_output(model, beta, num_diffusion_steps=20):
    ForwardDiffuser = GreyscaleDynamicForwardDiffuser() # Set up the dynamic forward diffuser, just for its embedding functions
