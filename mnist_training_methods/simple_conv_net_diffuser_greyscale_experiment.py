...
from greyscale_reverse_diffusion_training import SimpleConvNetDiffuser, load_non_noisy_data,cosine_beta_schedule,train_model, test_model, visualize_model_output

TRAINING_METHODS_DIR = os.path.dirname(os.path.abspath(__file__))
REPO_DIR = os.path.dirname(TRAINING_METHODS_DIR)
TRAINING_DATA_METHODS_DIR = os.path.join(REPO_DIR, 'mnist_training_data_methods')
...
def train_experiment():
    ...
    train_model(model, X_train_batches, y_train_batches, generator, betas, num_diffusion_steps=T, num_epochs=2, learning_rate=0.0001, model_save_filename=model_save_filename)
